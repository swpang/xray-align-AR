...
class VITONDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for key in self.c_names:
            ...
            c[key] = transforms.Resize(self.load_width, interpolation=Image.BILINEAR)(c[key])
            cm[key] = Image.open(osp.join(self.data_path, 'cloth-mask', c_name[key]))
            cm[key] = transforms.Resize(self.load_width, interpolation=Image.NEAREST)(cm[key])
            ...
            cloth_kp_name = c_name[key].replace('.jpg', '.json')
            ...
            c_kp[key] = self.transform(cloth_keypoints_map)

            if self.is_alias is True:
                warped_c[key] = Image.open(osp.join(self.data_path, 'warped_cloth', c_name[key])).convert('RGB')
                warped_c[key] = transforms.Resize(self.load_width, interpolation=Image.BILINEAR)(warped_c[key])
                warped_cm[key] = Image.open(osp.join(self.data_path, 'warped_cloth-mask', c_name[key]))
                warped_cm[key] = transforms.Resize(self.load_width, interpolation=Image.NEAREST)(warped_cm[key])
        ...
        pose_rgb = transforms.Resize(self.load_width, interpolation=Image.BILINEAR)(pose_rgb)
        ...
        pose_keypoints_map = self.transform(pose_keypoints_map)
        ...
        img = transforms.Resize(self.load_width, interpolation=Image.BILINEAR)(img)
